...
class OrderBook:
    ...
    async def insert_ask_order(self, place_order: Order):
        return await self.r.zadd(self.ask_set, {place_order.order_price: place_order.order_id})
    
    async def insert_bid_order(self, place_order: Order):
        return await self.r.zadd(self.bid_set, {place_order.order_price: place_order.order_id})

    async def remove_ask_order(self, place_order: Order):
        return await self.r.zrem(self.ask_set, {place_order.order_price: place_order.order_id})
    
    async def remove_bid_order(self, place_order: Order):
        return await self.r.zrem(self.bid_set, {place_order.order_price: place_order.order_id})
    
    async def generate_trade_pair(self) -> tuple(int|None, int|None):
        """
        ...
        try: 
            ...
            if min_ask_pair[1] > max_bid_pair[1]:
                await self.r.zadd(self.ask_set, min_ask_pair)
        ...
        min_ask_order_id = min_ask_pair[0]
        max_bid_order_id = max_bid_pair[0]
        
        return min_ask_order_id, max_bid_order_id

    async def preview_orderbook(self) -> Dict[str, List[int]]:
        max_bid_pairs = await self.r.zrevrange(self.bid_set, -5, -1)
        min_ask_pairs = await self.r.zrange(self.ask_set, 0, 4)
        return {
            "bids": max_bid_pairs,
            "asks": min_ask_pairs
        }
